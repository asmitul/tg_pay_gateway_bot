...
- GitHub Actions `release.yml` runs on successful `CI` workflow completions for `main`, logs into GHCR using `GITHUB_TOKEN`, and builds/pushes images under `ghcr.io/${{ github.repository }}` tagged with the triggering commit SHA plus `:main` and `:latest`; publishes commit/image tags and the image digest as job outputs.
