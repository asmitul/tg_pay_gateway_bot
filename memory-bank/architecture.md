...
## CI/CD
- GitHub Actions `ci.yml` enforces `go fmt ./...`, `go test ./...`, and `go build ./cmd/bot` on pushes/PRs to `main` and version tags (`v*`) with module caching and test log artifacts.
- GitHub Actions `release.yml` runs on successful `CI` workflow completions for `main` or `v*` tags, logs into GHCR using `GITHUB_TOKEN`, and builds/pushes images under `ghcr.io/${{ github.repository }}` tagged with the triggering commit SHA plus `:main`/`:latest` (when branch is `main`) and a version tag when the ref matches `v*`; publishes commit/image tags and the image digest as job outputs.
- GitHub Actions `production-deploy.yml` runs after a successful `Release` workflow on `main`, pulls the GHCR image tag for the commit SHA, deploys to the VPS via `appleboy/ssh-action` with rollback-on-failure semantics (stop/rename old container, start new, then clean up old on success), and uses `concurrency: production-deploy` to serialize runs; expects secrets `VPS_HOST`/`VPS_USER`/`VPS_PORT`/`SSH_KEY` plus runtime bot secrets (`TELEGRAM_TOKEN`, `BOT_OWNER`, `MONGO_URI`, optional `LOG_LEVEL`, and `MONGO_DB` defaulting to repo var or name).
